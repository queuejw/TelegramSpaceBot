import asyncio
import random

from core import constants
from core.manager import save_manager
from game.classes.player_ship import Ship
from game.game_main import is_game_active, ALL_PLAYERS, clamp


def get_new_ship_speed(ship: Ship) -> int:
    if ship.fuel < 1:
        return random.randint(100, 199)
    return clamp(100, 9999, random.randrange(ship.speed // 4, int(ship.speed * 1.25)))


# Основной цикл игры.
async def main_game_cycle(chat_id: int):
    while is_game_active(chat_id):
        player_ship: Ship = ALL_PLAYERS[chat_id]
        player_ship.speed = get_new_ship_speed(player_ship)

        # Изменение переменных завершено.
        ALL_PLAYERS[chat_id] = player_ship
        if constants.DEBUG_MODE:
            print(f"Основной цикл для чата {chat_id} завершён")
        save_manager.save_ship_state(chat_id, player_ship.export_as_dict())
        await asyncio.sleep(20)
